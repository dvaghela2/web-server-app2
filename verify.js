...
async function connectAndFetch() {
  try {
    await client.connect(); // Establishing the connection

    const database = client.db("mongodatabase");
    const collections = database.collection("web322");

    const result = await collections.find({}).toArray();
    console.log(result); // Handle or log the query result here

    // Any other database operations can be performed here
